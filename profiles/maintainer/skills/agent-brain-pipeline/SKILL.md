---
name: agent-brain-maintenance
description: Inspect scheduled refresh runs and repository update health.
version: 0.1.0
...
Use `brain-api list` to inspect runs created with the `maintainer` trigger and `brain-api status <RUN_ID>` for details. The scheduler uses the interval and jitter in `config/runtime.yaml`. Never overwrite prior commit-addressed raw snapshots. Use retry only for a diagnosed failed refresh.
